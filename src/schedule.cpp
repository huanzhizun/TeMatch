...
Schedule::Schedule(const Pattern& pattern, bool &is_pattern_valid, int performance_modeling_type, int restricts_type, bool use_in_exclusion_optimize ,int v_cnt, unsigned int e_cnt, long long tri_cnt)
{
    ...
    directed_adj_mat = new int[size * size];
    ...
    memcpy(directed_adj_mat, pattern.directed_adj_mat, size * size * sizeof(int));

    std::vector< std::pair<int,int> > best_pairs;
    best_pairs.clear();

    match_order = new int[size];
    for(int i=0;i<size;i++) match_order[i]=i;

    output_order = new int[size];
    for(int i=0;i<size;i++) output_order[i]=i;
    ...
    if( performance_modeling_type != 0) { 
...
#ifdef UNDIRECT_PATTERN
        remove_invalid_permutation(candidate_permutations);
#else
        remove_invalid_topo_permutation(candidate_permutations);
#endif
...
#else
                max_val = std::max(max_val, get_vec_optimize_num_topo(vec));
#endif
            }
...
#else
                if( get_vec_optimize_num_topo(vec) == max_val)
#endif
                {
                    tmp.push_back(vec);
                }
        ...
        for(const auto &vec : candidate_permutations) {
            ...
            if(restricts_type == 1) {
                ...
                restricts_vector.clear();
                if(pairs.size()) restricts_vector.push_back(pairs);
            }
        ...
        for(int i = 0; i < size; ++i)
            for(int j = 0; j < size; ++j){
                ...
                directed_adj_mat[INDEX(rank[i], rank[j], size)] = pattern.directed_adj_mat[INDEX(i, j, size)];
            }
        
        memcpy(match_order, best_order, sizeof(int)*size);
        for(int i=0;i<size;i++) output_order[match_order[i]] = i;
        printf("best order: ");
        for(int i=0;i<size;i++) printf("%d ", match_order[i]);
        printf("\n");
        delete[] best_order;
    }
    else {
    ...
    inVertexNum = new int[this->size];
    memset(inVertexNum, 0, sizeof(int)*this->size);

    inVertex = new int*[this->size];
    for(int i=0;i<this->size;i++){
        inVertex[i] = new int[this->size];
        memset(inVertex[i], -1, sizeof(int)*this->size);
    }
    ...
    for (int i = 1; i < size; ++i)
    {
        ...
        for (int j = 0; j < i; ++j)
            if (adj_mat[INDEX(j, i, size)])
            {
                inVertex[i][inVertexNum[i]++] = j; 
            }
    }

    for(int i=0;i<this->size;i++){
        printf("in vertex of %d: ", match_order[i]);
        for(int j=0; j<inVertexNum[i]; j++){
            printf("%d ", match_order[inVertex[i][j]]);
        }
        printf("\n");
    }
    ...
    for(int i=0;i<size;i++){
        for(int j=0;j<size;j++){
            if(adj_mat[INDEX(i,j,size)]){
                edges.push_back(std::make_pair(match_order[i],match_order[j]));
            }
        }
    }
    ...
    if(pattern.label){
        label = new int[size];
        for(int i=0;i<size;i++) label[i] = pattern.label[i];
    }
...
void Schedule::pathGenerate(int node, std::vector<int>& path, bool* vis){

    int hasNeb = 0;
    for(int i=0;i<size;i++){
        if(adj_mat[INDEX(node, i, size)]){
            hasNeb=1;
            path.push_back(match_order[i]);
            pathGenerate(i, path);
            path.pop_back();
        }
    }

    if(hasNeb==0 /* && path.size() > 2 */){
...
Schedule::Schedule(const int* _adj_mat, int _size)
{
    ...
    memcpy(directed_adj_mat, _adj_mat, size * size * sizeof(int));
...
int Schedule::get_vec_optimize_num_topo(const std::vector<int> &vec) {
    bool is_valid = true;
    for(int i = 1; i < size; ++i) {
        bool have_edge = false;
        for(int j = 0; j < i; ++j)
            if( adj_mat[INDEX(vec[j], vec[i], size)]) {
                have_edge = true;
                break;
            }
        if( have_edge == false) {
...
void Schedule::remove_invalid_topo_permutation(std::vector< std::vector<int> > &candidate_permutations) {

    int inDgree[size] = {0};

    memset(inDgree, 0, sizeof(int)*size);

    for(int i=0;i<size;i++){
        for(int j=0;j<size;j++){
            if(adj_mat[INDEX(i,j,size)]) inDgree[j]++;
        }
    }
    ...
    for(unsigned int i = 0; i < candidate_permutations.size(); ) {
        ...
        int temDgree[size];
        memcpy(temDgree, inDgree, sizeof(int)*size);

        for(int i=0;i<size;i++){
            assert(temDgree[vec[i]]>=0);
            if(temDgree[vec[i]]!=0){
                tag = false;
                break;
            }
                ...
                for(int j=0;j<size;j++){
                    if(adj_mat[INDEX(vec[i],j,size)]) temDgree[j]--;
                }
            }
        }
