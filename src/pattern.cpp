...
Pattern::Pattern(int _size, bool clique)
{
    ...
    memset(directed_adj_mat, 0, size * size * sizeof(int));
    ...
    memcpy(directed_adj_mat, adj_mat, size*size*sizeof(int));
}

Pattern::Pattern(int _size, char *buffer, int *labels) {
    size = _size;
    adj_mat = new int[size * size];
    directed_adj_mat = new int[size*size];
    ...
    if(labels){
        label = new int[size];
        memset(label, -1, sizeof(int)*size);
        for(int i=0;i<size;i++) label[i] = labels[i];
        printf("label: ");
        for(int i=0;i<size;i++) printf("%d ", label[i]);
        printf("\n");
    }
    ...
    for(int i = 0; i < size; ++i)
        for(int j = 0; j < size; ++j)
            if(buffer[INDEX(i,j,size)] == '1')
                // add_edge(i,j);
                add_ordered_edge(i,j);
    
    for(int i = 0; i < size; ++i)
        for(int j = 0; j < size; ++j)
            if(buffer[INDEX(i,j,size)] == '1')
                directed_adj_mat[INDEX(i, j, size)] = 1;

    printf("adj_mat: ");
    for(int i=0;i<size*size;i++) printf("%d",adj_mat[i]);
    printf("\ndirected_adj_mat: ");
    for(int i=0;i<size*size;i++) printf("%d",directed_adj_mat[i]);
    printf("\n");
}
...
Pattern::Pattern(const Pattern& p)
{
    ...
    memcpy(directed_adj_mat, p.directed_adj_mat, size * size * sizeof(int));
}
