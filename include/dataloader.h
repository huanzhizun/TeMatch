...
const long long Self_def_cnt = 0LL;

class DataLoader {
public:

    bool load_data(Graph* &g, DataType type, const char* path, int oriented_type = 0);
    bool load_data(Graph* &g, DataType type, std::string path, int oriented_type = 0, int LoadType=1);
        // LoadType == 0, nothing
        //          == 1  load temporal infomation
    bool load_data(Graph* &g, const std::vector<std::pair<uint32_t, uint32_t>>& edges, int oriented_type = 0);
...
private:
    ...
    bool general_load_data(Graph* &g, DataType type, std::string path, int oriented_type = 0);
    bool general_load_data(Graph *&g, DataType type, std::string path, int oriented_type, int LoadType=1);
