...
public:
    ...
    PathTree *ptree = nullptr;
    TeGraph *tg = nullptr;

    int* directed_adj_mat;
    ...
    int *match_order=nullptr;   // new label -> old label
    int *output_order;  // old label -> new label
    ...
    std::vector<std::vector<int>> extraPaths;

    std::vector<std::pair<int, int>> edges;

    void pathGenerate(int node, std::vector<int>& path, bool* vis=nullptr);
    ...
    int get_vec_optimize_num_topo(const std::vector<int> &vec);

    void remove_invalid_permutation(std::vector< std::vector<int> > &candidate_permutations);

    void remove_invalid_topo_permutation(std::vector< std::vector<int> > &candidate_permutations);
